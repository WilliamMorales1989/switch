package com.ec.tvcable.switchaprov.CreacionOtros;
...
public class CreacionOtrosBean implements CreacionOtros {
	...
	private Respuesta creacion (Aprovisionamiento_Type parametros){
		...
		if (equipoexistente.equals("OK")){
			...
			respuesta = CreacionEquiposOtros(parametros, resp, idcontroller);
	...
	private Respuesta CreacionEquiposOtros (Aprovisionamiento_Type parametros, Respuesta respuesta, int idcontroller){
		
		Respuesta resp = new Respuesta();
		
		try {
			connection = datasource.getConnection();
			CallableStatement prepareCall = connection.prepareCall("{call YPKG_SGR.YPRD_CREACION_EQUIPOS_OTROS(?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");
